...
class Pipeline(gst.Pipeline):
	def __init__(self,filename, fretboard):
		...
		# create spectrum bin
		#self.specbin = gst.Bin()
		#self.add(self.specbin)
		#self.compconvert.link(self.specbin)

		# create spectrum
		self.spectrum = gst.element_factory_make("spectrum", "spectrum")
		self.spectrum.set_property("bands",BANDS)
		self.spectrum.get_pad("sink").connect("notify::caps",self.notify_caps)
...
#		self.spectrum.link(self.caps)
	...
	def notify_caps(self,pad,caps):
		print "negotiated_caps", pad.get_negotiated_caps()[0]["rate"]

	def on_message(self, bus, message):
		s = message.structure
		if not s: return True
		#print [(s.nth_field_name(i),s[s.nth_field_name(i)]) for i in xrange(s.n_fields())]
	...
	def OnPad(self, comp, pad):
		...
		print "audiofreq",self.AUDIOFREQ
...
#	pl = Pipeline("/home/maxi/Musik/ogg/jamendo_track_9087.ogg",fretboard)
	pl = Pipeline("/home/maxi/test/once/i.wav",fretboard)
